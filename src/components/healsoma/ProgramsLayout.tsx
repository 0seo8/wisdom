      ...
      {PROGRAMS.map((program, index) => (
        <section
          ...
          className={`scroll-mt-[76px] bg-[#FCF3EB] ${
            index === 0 ? "lg:pt-0 lg:pb-7" : "lg:py-7"
          }`}
        >
          <div className="mx-auto max-w-[1200px] px-0 py-0 lg:max-w-none lg:px-0">
            ...
            <div className="hidden items-center md:flex lg:hidden">
              ...
              >
                <img
                  src={program.image}
                  alt=""
                  className="block h-auto w-full px-[10px]"
                  loading="lazy"
                />
              </div>
              <div
                className={`w-1/2 shrink-0 px-[30px] py-4 text-left ${
            ...
            <DesktopProgramSection program={program} />
          </div>
        </section>
      ))}
    </div>
...
function DesktopProgramSection({ program }: { program: ProgramEntry }) {
  const textBlock = (
    <div className="px-[10px]" style={{ paddingTop: program.desktopTextTop }}>
      <DesktopTextBlock
        title={program.desktopTitle ?? program.title}
        lines={program.desktopTextLines}
        footLines={program.desktopFootLines}
        bodyAlign={program.desktopBodyAlign}
      />
    </div>
  );
  ...
  return (
    <div
      ...
      style={{ gridTemplateColumns: program.desktopGridColumns }}
    >
      <div />
      {program.desktopReverse ? textBlock : imageBlock}
      {program.desktopReverse ? imageBlock : textBlock}
      <div />
    </div>
  );
}
...
function DesktopTextBlock({
  ...
  return (
    <div>
      <h2 className="text-center font-['Times_New_Roman',Times,serif] text-[45px] font-semibold tracking-[-1px] text-[#541404]">
        {title}
      </h2>
      <div
        className={`mt-3 font-['Noto_Sans_KR',sans-serif] text-[17px] font-normal leading-[1.7] tracking-[-0.4px] text-[#333333] ${
